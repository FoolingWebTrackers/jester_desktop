<template>
  ...
  <div class="persona-container">
    <div v-for="persona in personas" :key="persona.id"
      :class="['persona-box', { selected: persona === this.selectedPersona }]">
      <img :src="persona.photo" alt="Persona Photo" class="persona-photo" @error="setDefaultPhoto(persona)" />
      <div class="persona-info">
        <h3 class="persona-name unselectable">{{ persona.name }}</h3>
        <p v-if="showDescription" class="persona-desc unselectable"
          :style="{ maxHeight: this.windowHeight >= 900 ? '100px' : '60px' }">{{ persona.description }}</p>
      </div>
...
export default {
  ...
  methods: {
    ...
    deselectPersona() {
      ...
      const sendRequest = async () => {
        const url = "http://localhost:3000" + "/stop"; // Replace PORT with your server's port
        const response = await fetch(url, {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
...
@media (min-width: 1000px) {
  .persona-box {
    width: calc(15% - 10px);
    /* 4 per row for even larger screens */
  }
}
...
.persona-desc::-webkit-scrollbar-track {
  background: transparent;
  /* Invisible track for a minimal look */
}
...
.persona-desc::-webkit-scrollbar-thumb:hover {
  background: #24262b;
  /* Slightly brighter on hover for interaction feedback */
}
...
.selected-button {
  align-self: center;
  /* Keep the button centered at the bottom */
  margin-top: auto;
