<template>
  ...
  export default {
    ...
    methods: {
      async getMarketplace(username) {
        try {
          const response = await fetch(this.pageUrl + "/get", {
            method: "POST",
            headers: {
      ...
      async buyPersona(username, personaName) {
        try {
          const response = await fetch(this.pageUrl + "/buyPersona", {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
    ...
    mounted() {
      this.getMarketplace(globalState.username);
