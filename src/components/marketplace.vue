...
export default {
  ...
  methods: {
    async getUserMarketplace(username) {
      try {
        const response = await fetch(this.pageUrl + "/marketplace/items", {
          method: "POST",
          headers: {
    ...
    buyPersona(persona) {
      const sendRequest = async () => {
        const url = this.pageUrl + "/marketplace/buy-persona"; // Replace PORT with your server's port
  ...
  mounted() {
    this.getUserMarketplace(globalState.username);
